...
async fn run() -> AnyResult<()> {
    ...
    //allow the server to listen to anything. Safe once we run this in docker.
...
/// Grabs the database name, postgres user and postgres users password from the CLA to set the .env variable for the db connection
#[tokio::main]
